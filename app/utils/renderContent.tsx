...
export const renderContent = (result: BlockObjectWithChildren) => {
  return match(result)
    ...
    .with({ type: "breadcrumb" }, (res) => {
      return <p key={res.id}>Not support</p>;
    })
    ...
    .with({ type: "callout" }, (res) => {
      ...
      return (
        <div
          key={res.id}
          className="flex items-center gap-4 bg-neutral-1 p-4 dark:bg-neutral-7"
        >
          <span>{emoji}</span>
          {renderRichText(
            "p",
    ...
    .with({ type: P.union("column_list", "column") }, (res) => {
      const createColumnList = (children: BlockObjectWithChildren[]) => {
        return (
          <div
            key={res.id}
            className={res.type === "column_list" ? "flex" : "flex-1"}
          >
            {children.map((child) => {
    ...
    .with({ type: "divider" }, (res) => <hr key={res.id} />)
    .with({ type: "embed" }, (res) => {
      return <p key={res.id}>Not support</p>;
    })
    ...
    .with({ type: "heading_1" }, (res) =>
      renderRichText(
        "h1",
        getTextContent(res.heading_1.rich_text[0] as TextRichTextItemResponse),
        res.id,
      ),
    )
    .with({ type: "heading_2" }, (res) =>
      renderRichText(
        "h2",
        getTextContent(res.heading_2.rich_text[0] as TextRichTextItemResponse),
        res.id,
      ),
    )
    .with({ type: "heading_3" }, (res) =>
      renderRichText(
        "h3",
        getTextContent(res.heading_3.rich_text[0] as TextRichTextItemResponse),
        res.id,
      ),
    )
    ...
    .with({ type: "paragraph", paragraph: { rich_text: [P._] } }, (res) =>
      renderRichText(
        "p",
        getTextContent(res.paragraph.rich_text[0] as TextRichTextItemResponse),
        res.id,
      ),
    )
    ...
    .with({ type: "quote" }, (res) =>
      renderRichText(
        "blockquote",
        getTextContent(res.quote.rich_text[0] as TextRichTextItemResponse),
        res.id,
        "border-0 border-l-2 border-neutral-2 border-solid pl-4 dark:border-neutral-7",
      ),
    )
    ...
    .with({ type: "toggle" }, (res) => {
      return (
        <details key={res.id}>
          {renderRichText(
            "summary",
            getTextContent(res.toggle.rich_text[0] as TextRichTextItemResponse),
            res.id,
            "cursor-pointer",
          )}
          <div className="p-l-3.5">
